...
from unitedarabemirates_crawler.sites.wiza.contact_selector import WizaContactSelector
...
class UaeBasicTestCase(unittest.TestCase):
    ...
    def test_wiza_selector_prefers_ceo_over_other_cxo(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            selector = WizaContactSelector(Path(tmp_dir))
            try:
                result = selector.pick_representative(
                    "Example Company",
                    [
                        {"full_name": "Alice Finance", "job_title": "Chief Financial Officer", "job_title_levels": ["CXO"]},
                        {"full_name": "Bob Chief", "job_title": "Chief Executive Officer", "job_title_levels": ["CXO"]},
                    ],
                )
            finally:
                selector.close()
        self.assertEqual(result, "Bob Chief")

    def test_wiza_selector_rejects_ceo_office_false_positive(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            selector = WizaContactSelector(Path(tmp_dir))
            try:
                result = selector.pick_representative(
                    ...
                    [
                        {
                            "full_name": "Pooja Punjabi",
                            "job_title": "Strategic Projects, Chief Executive Officer's Office",
    ...
    def test_wiza_selector_rejects_ceo_office_assistance_false_positive(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            selector = WizaContactSelector(Path(tmp_dir))
            try:
                result = selector.pick_representative(
                    ...
                    [
                        {
                            "full_name": "Umar Osg",
                            "job_title": "Chief Executive Officer Office Assistance",
                            "job_title_levels": ["CXO"],
    ...
    def test_wiza_selector_prefers_uae_ceo_when_multiple_generic_ceos_exist(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            selector = WizaContactSelector(Path(tmp_dir))
            try:
                result = selector.pick_representative(
                ...
                selector.close()
        self.assertEqual(result, "Bob Local")

    def test_wiza_selector_rejects_misspelled_non_ceo_abbreviation(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            selector = WizaContactSelector(Path(tmp_dir))
            try:
                result = selector.pick_representative(
    ...
    def test_wiza_selector_accepts_co_chief_executive_officer(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            selector = WizaContactSelector(Path(tmp_dir))
            try:
                result = selector.pick_representative(
                    ...
                    [
                        {
                            "full_name": "Dual Leader",
                            "job_title": "Co-Chief Executive Officer",
                            "job_title_levels": ["CXO"],
                            "location_name": "Abu Dhabi, United Arab Emirates",
                        }
                    ],
                ...
                selector.close()
        self.assertEqual(result, "Dual Leader")

    def test_wiza_selector_prefers_chairman_and_chief_executive_over_generic_ceo(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            selector = WizaContactSelector(Path(tmp_dir))
            try:
                result = selector.pick_representative(
                    ...
                    [
                        {
                            "full_name": "Chair Leader",
                            "job_title": "Chairman And Chief Executive, Example Airline & Group",
                            "job_title_levels": ["CXO"],
                            "location_name": "Dubai, United Arab Emirates",
                        },
                        {
                ...
                selector.close()
        self.assertEqual(result, "Chair Leader")
