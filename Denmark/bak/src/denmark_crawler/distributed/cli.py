"""丹麦静态切片与合并 CLI。"""

from __future__ import annotations

import argparse
from pathlib import Path


ROOT = Path(__file__).resolve().parents[3]


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="丹麦静态切片与合并命令")
    sub = parser.add_subparsers(dest="command", required=True)

    plan_dnb = sub.add_parser("plan-dnb", help="生成 DNB 静态切片")
    plan_dnb.add_argument("--output-dir", default=str(ROOT / "output" / "distributed" / "dnb"))
    plan_dnb.add_argument("--country", default="dk")
    plan_dnb.add_argument("--shards", type=int, required=True)

    bootstrap_dnb = sub.add_parser("bootstrap-dnb", help="把旧 DNB store 迁移到 shard run 目录")
    bootstrap_dnb.add_argument("--legacy-db", default=str(ROOT / "output" / "dnb" / "store.db"))
    bootstrap_dnb.add_argument("--shard-dir", default=str(ROOT / "output" / "distributed" / "dnb"))
    bootstrap_dnb.add_argument("--output-root", default=str(ROOT / "output" / "runs"))

    merge_site = sub.add_parser("merge-site", help="合并站点产物")
    merge_site.add_argument("site", choices=["dnb"])
    merge_site.add_argument("--run-dir", action="append", required=True)
    merge_site.add_argument("--output-dir", default="")

    return parser


def _default_site_output(site: str) -> Path:
    return ROOT / "output" / "dnb"


def run_dist(argv: list[str]) -> int:
    args = _build_parser().parse_args(argv)
    if args.command == "plan-dnb":
        from denmark_crawler.distributed.dnb_planner import plan_dnb_shards

        summary = plan_dnb_shards(
            args.output_dir,
            shard_count=max(int(args.shards), 1),
            country_iso_two_code=str(args.country).strip().lower() or "dk",
        )
        print(
            "丹麦 DNB 静态切片完成：shards={shards} total={total}".format(
                shards=int(summary["shard_count"]),
                total=int(summary["total_segments"]),
            )
        )
        print(f"目录：{summary['output_dir']}")
        return 0
    if args.command == "bootstrap-dnb":
        from denmark_crawler.distributed.bootstrap import bootstrap_dnb_shards

        summary = bootstrap_dnb_shards(
            args.legacy_db,
            args.shard_dir,
            args.output_root,
        )
        print(
            "丹麦 DNB 旧进度迁移完成：shards={shards}".format(
                shards=int(summary["shard_count"]),
            )
        )
        print(f"目录：{args.output_root}")
        return 0
    if args.command == "merge-site":
        from denmark_crawler.distributed.site_merge import merge_site_runs

        output_dir = Path(args.output_dir).resolve() if str(args.output_dir).strip() else _default_site_output(args.site)
        summary = merge_site_runs(list(args.run_dir), output_dir)
        print(
            "丹麦站点合并完成：records={records} merged={merged}".format(
                records=int(summary["input_records"]),
                merged=int(summary["merged_companies"]),
            )
        )
        print(f"目录：{summary['output_dir']}")
        return 0
    raise ValueError(f"不支持的 dist 命令: {args.command}")

