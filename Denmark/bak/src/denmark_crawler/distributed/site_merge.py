...
def _refresh_run_snapshot(run_dir: Path) -> None:
    ...
    if "dnb_segments" in tables:
        from denmark_crawler.dnb.runtime.snapshot_export import export_jsonl_snapshots
...
def _record_key(record: dict[str, object]) -> str:
    company_name = "".join(ch for ch in str(record.get("company_name", "")).strip().lower() if ch.isalnum())
    if company_name:
