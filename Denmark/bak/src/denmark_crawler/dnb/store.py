...
from denmark_crawler.dnb.naming import resolve_company_name
from denmark_crawler.dnb.runtime.snapshot_export import export_jsonl_snapshots as export_snapshot_files
...
class DnbDenmarkStore:
    """基于 sqlite 的丹麦 DNB 断点与队列存储。"""
    ...
    def ensure_discovery_seed(self, segment_id: str, expected_count: int = 0) -> None:
        ...
        target_country = target_parts[1] if len(target_parts) >= 2 else "dk"
        with self._lock:
    ...
    def upsert_company_listing(self, record: dict[str, str]) -> None:
        self._upsert_company(
            ...
            country=record["country"] or "Denmark",
        ...
        with self._lock:
            def _op() -> dict[str, object]:
                ...
                row = {
                    ...
                    "country": _merge_text(current.get("country", "Denmark") if current else "Denmark", country),
