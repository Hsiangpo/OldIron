...
class ProffStore:
        ...
        with self._lock:
            ...
            self._conn.execute(
                ...
                INSERT INTO companies(
                    orgnr, company_name, representative, representative_role, homepage, domain,
                    phone, emails_json, source_query, source_page, source_url, raw_json,
    ...
    def mark_gmap_done(self, *, orgnr: str, website: str, source: str, phone: str) -> None:
        _ = source
        now = _utc_now()
        with self._lock:
            current = self._fetch_company_locked(orgnr) or {}
            final_website = str(website or "").strip() or str(current.get("homepage", "")).strip()
            final_phone = str(phone or "").strip() or str(current.get("phone", "")).strip()
            self._conn.execute(
                """
                UPDATE companies
                SET homepage = ?, domain = ?, phone = ?, gmap_status = 'done', last_error = '', updated_at = ?
                WHERE orgnr = ?
                """,
                (final_website, _extract_domain(final_website), final_phone, now, orgnr),
            )
            self._conn.execute(
    ...
    def mark_firecrawl_done(self, *, orgnr: str, emails: list[str], retry_after_seconds: float = 0.0) -> None:
        now = _utc_now()
        with self._lock:
            current = self._fetch_company_locked(orgnr) or {}
            ...
            self._conn.execute(
                """
                UPDATE companies
                SET emails_json = ?, firecrawl_status = ?, firecrawl_retry_at = ?, last_error = '', updated_at = ?
                WHERE orgnr = ?
                """,
                (
    ...
    def refresh_final_company(self, orgnr: str) -> None:
        with self._lock:
            ...
            if company_name and representative and emails:
                self._conn.execute(
                    """
                    INSERT INTO final_companies(orgnr, company_name, ceo, homepage, contact_emails, domain, phone, updated_at)
                    VALUES(?, ?, ?, ?, ?, ?, ?, ?)
                    ON CONFLICT(orgnr) DO UPDATE SET
                        company_name = excluded.company_name,
                        ceo = excluded.ceo,
    ...
    def _schedule_company_tasks_locked(self, orgnr: str) -> None:
        ...
        homepage = str(record.get("homepage", "")).strip()
        now = _utc_now()
        if not representative or emails:
        ...
        if homepage:
            ...
            return
        if str(record.get("gmap_status", "")).strip() in {"", "failed"}:
