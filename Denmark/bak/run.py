...
站点：
  ...
  virk   — datacvr.virk.dk 丹麦企业公开接口（优先直拿邮箱与代表人）
  proff  — proff.dk 搜索页直取公司名 / 法人 / 邮箱 / 官网
...
VIRK_REQUIRED_MODULES = (
    ("curl_cffi", "curl_cffi"),
    ("requests", "requests"),
    ("websocket", "websocket-client"),
    ("openai", "openai"),
)
PROFF_REQUIRED_MODULES = (
    ("requests", "requests"),
    ("curl_cffi", "curl_cffi"),
    ("openai", "openai"),
)
...
def _required_modules_for_command(site: str) -> tuple[tuple[str, str], ...]:
    ...
    if normalized == "virk":
        return BASE_REQUIRED_MODULES + VIRK_REQUIRED_MODULES
    if normalized == "proff":
        return BASE_REQUIRED_MODULES + PROFF_REQUIRED_MODULES
...
def _dispatch(argv: list[str]) -> int:
    ...
    if site == "dnb":
        ...
        return run_dnb(rest)
    if site == "virk":
        from denmark_crawler.virk.cli import run_virk
