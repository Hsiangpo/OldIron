...
from denmark_crawler.dnb.browser_cookie import DnbCookieProvider
from denmark_crawler.dnb.client import DnbClient
from denmark_crawler.dnb.config import DnbDenmarkConfig
from denmark_crawler.dnb.pipeline import run_dnb_pipeline
    ...
    for _ in range(8):
        try:
            ...
            if lock_pid and lock_pid != pid and _pid_exists(lock_pid, created_at=lock_created_at):
                raise RuntimeError(f"已有运行中的丹麦 DNB 进程: PID={lock_pid}")
            try:
                lock_path.unlink()
        ...
        return lock_path
    raise RuntimeError("无法获取丹麦 DNB 运行锁，请稍后重试。")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="丹麦 DNB 协议爬虫（邮箱阶段默认 Firecrawl）")
...
def run_dnb(argv: list[str]) -> int:
    ...
    config = DnbDenmarkConfig.from_env(
        project_root=ROOT,
    ...
    if not cookie_header:
        raise RuntimeError("丹麦 DNB 启动失败：9222 浏览器未提供 DNB cookie。")
