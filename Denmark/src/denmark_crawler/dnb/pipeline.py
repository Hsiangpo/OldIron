...
from denmark_crawler.dnb.catalog import INDUSTRY_CATEGORY_COUNT
from denmark_crawler.dnb.catalog import INDUSTRY_PAGE_COUNT
from denmark_crawler.dnb.catalog import INDUSTRY_SUBCATEGORY_COUNT
from denmark_crawler.dnb.catalog import build_root_seed_segments
from denmark_crawler.dnb.client import DnbClient
from denmark_crawler.dnb.client import extract_child_segments
from denmark_crawler.dnb.client import parse_company_listing
from denmark_crawler.dnb.client import parse_company_profile
from denmark_crawler.dnb.config import DnbDenmarkConfig
from denmark_crawler.dnb.runtime.detail_queue import DetailQueueStore
from denmark_crawler.dnb.runtime.detail_queue import DetailTask
from denmark_crawler.dnb.store import DnbDenmarkStore
from denmark_crawler.dnb.store import FirecrawlTask
from denmark_crawler.dnb.store import GMapTask
...
def _build_gmap_queries(task: GMapTask) -> list[str]:
    parts = [task.city, task.region, task.country or "Denmark"]
...
class DnbDenmarkPipelineRunner:
    """丹麦 DNB 流式执行器。"""

    def __init__(
        self,
        *,
        config: DnbDenmarkConfig,
        client: DnbClient,
        skip_dnb: bool,
        skip_gmap: bool,
        ...
        self.store = DnbDenmarkStore(config.store_db_path)
    ...
    def run(self) -> None:
        ...
        foreign_countries = self.store.foreign_country_counts("Denmark")
        if foreign_countries:
            ...
            if max_foreign_count >= 100:
                raise RuntimeError(
                    "Denmark/output/dnb/store.db 中检测到大批历史非丹麦数据，请先清理旧断点后重跑。"
                    f" 当前发现：{sample}"
                )
            logger.warning(
                "Denmark store 中存在少量非丹麦记录，继续运行：%s",
                sample,
            )
        ...
        if self.skip_dnb:
            ...
            for idx in range(self.config.dnb_pipeline_workers):
                workers.append(
                    threading.Thread(target=self._dnb_worker, name=f"DNB-DK-{idx+1}", daemon=True)
                )
            for idx in range(self.config.dnb_workers):
                workers.append(
                    threading.Thread(target=self._detail_worker, name=f"DNB-Detail-DK-{idx+1}", daemon=True)
                )
        if not self.skip_gmap:
            for idx in range(self.config.gmap_workers):
                workers.append(threading.Thread(target=self._gmap_worker, name=f"GMap-DK-{idx+1}", daemon=True))
        if not self.skip_firecrawl:
            for idx in range(self.config.snov_workers):
                workers.append(threading.Thread(target=self._firecrawl_worker, name=f"Firecrawl-DK-{idx+1}", daemon=True))
    ...
    def _record_from_dict(self, payload: dict[str, str]):
        from denmark_crawler.dnb.models import CompanyRecord
...
def run_dnb_pipeline(
    *,
    ...
    runner = DnbDenmarkPipelineRunner(
        config=config,
        client=client,
        skip_dnb=skip_dnb,
        skip_gmap=skip_gmap,
