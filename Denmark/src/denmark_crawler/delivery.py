...
def build_delivery_bundle(data_root: Path, delivery_root: Path, day_label: str) -> dict[str, object]:
    ...
    day, _latest = validate_day_sequence(Path(delivery_root), "Denmark", day_label)
...
def _load_baseline_keys(delivery_root: Path, baseline_day: int) -> set[str]:
    """加载前一天交付的公司去重键集合（使用 keys.txt —— 它包含截至该日的全部 key）。"""
    if baseline_day <= 0:
        return set()
    keys_path = Path(delivery_root) / f"Denmark_day{baseline_day:03d}" / "keys.txt"
    if keys_path.exists():
        return set(keys_path.read_text(encoding="utf-8").strip().splitlines())
    # 兼容旧版：如果 keys.txt 不存在，回退到 companies.csv
