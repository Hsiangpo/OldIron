...
def _looks_suspicious_record(homepage: str, phone: str) -> bool:
    # 丹麦第一版先不过滤“疑似海外”，避免把合法公司误伤。
    _ = homepage, phone
    return False
...
def _list_existing_days(delivery_root: Path) -> list[int]:
    ...
    for item in delivery_root.iterdir():
        if not item.is_dir():
            continue
        matched = re.fullmatch(r"Denmark_day(\d{3})", item.name)
        if matched:
...
def _read_baseline_keys(delivery_root: Path, baseline_day: int) -> set[str]:
    ...
    if baseline_day <= 0:
        return set()
    baseline_dir = delivery_root / f"Denmark_day{baseline_day:03d}"
...
def _load_historical_baseline_records(
    ...
    for day in range(1, baseline_day + 1):
        csv_path = delivery_root / f"Denmark_day{day:03d}" / "companies.csv"
...
def build_delivery_bundle(
    ...
    for record in all_records:
        ...
        if company_name and ceo and has_emails:
            if not is_history_baseline and _looks_suspicious_record(homepage, phone):
                suspicious += 1
                continue
    ...
    # 构建交付目录
    day_dir = delivery_root / f"Denmark_day{target_day:03d}"
