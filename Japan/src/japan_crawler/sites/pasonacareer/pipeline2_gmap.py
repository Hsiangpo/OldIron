"""PasonaCareer Pipeline 2 — GMap 补官网。"""
...
LOGGER = logging.getLogger("pasonacareer.pipeline2")
...
def run_pipeline_gmap(
    *,
    ...
    LOGGER.info("PasonaCareer GMap：待处理 %d 家，并发=%d", len(pending), concurrency)
    ...
    def _worker(company: dict[str, str]) -> tuple[int, str]:
        ...
        try:
            ...
            return int(company["id"]), result.website if result else ""
    ...
    with ThreadPoolExecutor(max_workers=max(int(concurrency or 1), 1)) as executor:
        ...
        for future in as_completed(futures):
            row_id, website = future.result()
            processed += 1
            if website:
                store.update_website(row_id, website)
                found += 1
            else:
