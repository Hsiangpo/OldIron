...
def run_pipeline_email(
    *,
    ...
    settings.validate()
    LOGGER.info("PasonaCareer 邮箱提取：待处理 %d 家，并发=%d，批量=%d", len(pending), concurrency, batch_limit)
...
def _email_batch_limit(max_items: int, concurrency: int) -> int:
    if max_items > 0:
        return max_items
    configured = int(os.getenv("PASONACAREER_EMAIL_BATCH_SIZE", str(_DEFAULT_BATCH_SIZE)) or _DEFAULT_BATCH_SIZE)
