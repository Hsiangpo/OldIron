...
class EmailBatchingTests(unittest.TestCase):
    def test_bizmaps_batch_size_scales_with_concurrency(self) -> None:
        from japan_crawler.sites.bizmaps.pipeline3_email import _iter_batches
        from japan_crawler.sites.bizmaps.pipeline3_email import _resolve_batch_size

        self.assertEqual(512, _resolve_batch_size(128))
        batches = list(_iter_batches(list(range(1300)), 512))
        self.assertEqual([512, 512, 276], [len(batch) for batch in batches])
