...
LOGGER = logging.getLogger("germany.common.pipelines")
    ...
    *,
    ...
    def _get_client() -> GoogleMapsClient:
        ...
        if client is not None:
            return client
        thread_local.client = GoogleMapsClient(GoogleMapsConfig(hl="de", gl="de"))
        return thread_local.client

    def _worker(company: dict[str, str]) -> tuple[str, str, str]:
        query = f"{company['company_name']} {company.get('address', '')} Germany".strip()
