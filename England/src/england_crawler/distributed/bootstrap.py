...
from england_crawler.companies_house.snapshot_export import export_jsonl_snapshots as export_ch_snapshots
...
def _ch_keep_ids(shard_file: Path) -> set[str]:
    keep_ids: set[str] = set()
    for company_name in load_company_names_from_source(shard_file):
        normalized = normalize_company_name(company_name)
        if normalized:
            keep_ids.add(hashlib.md5(normalized.encode("utf-8")).hexdigest())
    return keep_ids
...
def _bootstrap_ch_store(legacy_db: Path, shard_file: Path, target_dir: Path) -> dict[str, object]:
    keep_ids = _ch_keep_ids(shard_file)
    target_db = target_dir / "store.db"
    _copy_sqlite_db(legacy_db, target_db)
    conn = sqlite3.connect(target_db)
    try:
        _load_keep_values(conn, "tmp_keep_companies", keep_ids)
        conn.execute(
            """
            DELETE FROM companies
            WHERE NOT EXISTS (
                SELECT 1 FROM tmp_keep_companies WHERE tmp_keep_companies.item_id = companies.comp_id
            )
            """
        )
        for table in ("ch_queue", "gmap_queue", "snov_queue"):
            conn.execute(
                ...
                DELETE FROM {table}
                WHERE NOT EXISTS (
                    SELECT 1 FROM tmp_keep_companies WHERE tmp_keep_companies.item_id = {table}.comp_id
                )
                """
            )
        ...
        conn.execute(
            ...
            VALUES(?, ?, ?, datetime('now'))
            """,
            (f"{shard_file.resolve()}|full", _file_fingerprint(shard_file), len(keep_ids)),
        )
        conn.commit()
    finally:
        conn.close()
    export_ch_snapshots(target_db, target_dir)
    return {
        "shard_file": str(shard_file),
        "output_dir": str(target_dir),
        "company_count": len(keep_ids),
    }


def bootstrap_companies_house_shards(
    legacy_db_path: str | Path,
    shard_dir: str | Path,
    output_root: str | Path,
) -> dict[str, object]:
    """把旧 CH store 切到各 shard run 目录。"""
    legacy_db = Path(legacy_db_path).resolve()
    if not legacy_db.exists():
        raise RuntimeError(f"未找到旧 CH store: {legacy_db}")
    shard_root = Path(shard_dir).resolve()
    output_dir = Path(output_root).resolve()
    manifest = _load_manifest(shard_root)
    shard_files = sorted(shard_root.glob("shard-*.txt"))
    items: list[dict[str, object]] = []
    for shard_file in shard_files:
        shard_number = _parse_shard_index(shard_file) + 1
        target_dir = output_dir / f"ch-shard-{shard_number:03d}"
        shutil.rmtree(target_dir, ignore_errors=True)
        items.append(_bootstrap_ch_store(legacy_db, shard_file, target_dir))
    return {
        "site": "companies-house",
        "legacy_db": str(legacy_db),
        "shard_count": int(manifest.get("shard_count", len(items)) or len(items)),
        "items": items,
    }
...
def _bootstrap_dnb_store(
    ...
    store = DnbEnglandStore(target_db)
    try:
