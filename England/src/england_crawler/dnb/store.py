...
from england_crawler.dnb.runtime.snapshot_export import export_jsonl_snapshots as export_snapshot_files
from england_crawler.dnb.runtime.sqlite_retry import run_with_sqlite_retry
...
class DnbEnglandStore:
    """基于 sqlite 的英国 DNB 断点与队列存储。"""
    ...
    def _init_schema(self) -> None:
        with self._lock:
            self._conn.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_dnb_discovery_claim ON dnb_discovery_queue(status, updated_at, segment_id);
                CREATE INDEX IF NOT EXISTS idx_dnb_segment_claim ON dnb_segments(status, updated_at, segment_id);
                CREATE INDEX IF NOT EXISTS idx_dnb_gmap_claim ON gmap_queue(status, next_run_at, updated_at, duns);
                CREATE INDEX IF NOT EXISTS idx_dnb_site_claim ON site_queue(status, next_run_at, updated_at, duns);
                CREATE INDEX IF NOT EXISTS idx_dnb_snov_claim ON snov_queue(status, next_run_at, updated_at, duns);
    ...
    def ensure_discovery_seed(self, segment_id: str, expected_count: int = 0) -> None:
        ...
        target_industry = target_parts[0] if len(target_parts) >= 1 else "construction"
        target_country = target_parts[1] if len(target_parts) >= 2 else "gb"
        with self._lock:
            ...
            root_row = self._conn.execute(
                """
                SELECT segment_id, industry_path, country_iso_two_code
            ...
            if root_row is not None:
                if (
                    str(root_row["industry_path"]).strip() != target_industry
                    or str(root_row["country_iso_two_code"]).strip() != target_country
                ):
                    now = _utc_now()
                    self._conn.execute(
                        """
                        UPDATE dnb_discovery_queue
                        SET industry_path = ?, country_iso_two_code = ?, region_name = '', city_name = '',
                            expected_count = ?, status = 'pending', updated_at = ?
                        WHERE segment_id = ?
                        """,
                        (target_industry, target_country, max(int(expected_count), 0), now, segment_id),
                    )
                    self._conn.commit()
                return
            if queued > 0 or stable > 0:
                target_rows = self._scalar(
                    f"""
                    SELECT COUNT(*) FROM (
                        SELECT country_iso_two_code FROM dnb_discovery_queue
                        UNION ALL
                        SELECT country_iso_two_code FROM dnb_segments
                    ) WHERE country_iso_two_code = '{target_country}'
                    """
                )
                if target_rows > 0:
                    return
                self._conn.execute("DELETE FROM dnb_discovery_queue")
            ...
            self._conn.execute(
                ...
                """,
                (segment_id, target_industry, target_country, max(int(expected_count), 0), now),
            )
            self._conn.commit()

    def ensure_discovery_seeds(self, seeds: list[dict[str, object]]) -> None:
        """批量补齐全站 discovery 种子，保留既有进度。"""
        now = _utc_now()
        rows: list[tuple[str, str, str, str, str, int, str]] = []
        for seed in seeds:
            segment_id = str(seed.get("segment_id", "")).strip()
            industry_path = str(seed.get("industry_path", "")).strip()
            country_iso_two_code = str(seed.get("country_iso_two_code", "")).strip()
            if not segment_id or not industry_path or not country_iso_two_code:
                continue
            rows.append(
                (
                    segment_id,
                    ...
                    str(seed.get("region_name", "")).strip(),
                    str(seed.get("city_name", "")).strip(),
                    max(int(seed.get("expected_count", 0) or 0), 0),
                    now,
                )
            )
        if not rows:
    ...
    def claim_segment(self, page_size: int, max_pages: int | None = None) -> SegmentCursor | None:
        with self._lock:
            row = self._conn.execute(
                """
    ...
    def next_segment(self, page_size: int, max_pages: int | None = None) -> SegmentCursor | None:
        return self.claim_segment(page_size, max_pages)
    ...
    def upsert_company_listing(self, record: dict[str, str]) -> None:
        self._upsert_company(
            ...
            country=record["country"] or "United Kingdom",
        ...
        with self._lock:
            def _op() -> dict[str, object]:
                ...
                row = {
                    ...
                    "country": _merge_text(current.get("country", "United Kingdom") if current else "United Kingdom", country),
                ...
                self._conn.commit()
                return row
            row = run_with_sqlite_retry(self._conn, _op)
    ...
    def foreign_country_counts(self, expected_country: str) -> list[tuple[str, int]]:
        country = str(expected_country or "").strip()
        with self._lock:
            rows = self._conn.execute(
                """
                SELECT country, COUNT(*) AS count
                FROM companies
                WHERE TRIM(country) != '' AND country != ?
                GROUP BY country
                ORDER BY count DESC, country ASC
                """,
                (country,),
            ).fetchall()
        return [(str(row["country"]).strip(), int(row["count"])) for row in rows]
    ...
    def enqueue_firecrawl_task(self, duns: str) -> None:
        self._enqueue_task("snov_queue", duns)
    ...
    def claim_firecrawl_task(self) -> FirecrawlTask | None:
        ...
        if row is None:
            return None
        homepage = str(row["website"]).strip() or str(row["dnb_website"]).strip()
    ...
    def _claim_task(self, table: str) -> sqlite3.Row | None:
        ...
        with self._lock:
            def _op() -> sqlite3.Row | None:
                ...
                self._conn.commit()
                return row
            return run_with_sqlite_retry(self._conn, _op)
    ...
    def mark_gmap_done(
        ...
        *,
        ...
        with self._lock:
            ...
            final_gmap_name = company_name_local_gmap.strip() or str(current.get("company_name_en_gmap", "")).strip()
            ...
            self._conn.commit()
        if final_domain:
            self.enqueue_firecrawl_task(duns)
        self.refresh_final_company(duns)
    ...
    def defer_firecrawl_task(self, *, duns: str, retries: int, delay_seconds: float, error_text: str) -> None:
        self._defer_task("snov_queue", duns=duns, retries=retries, delay_seconds=delay_seconds, error_text=error_text)
    ...
    def mark_firecrawl_failed(self, *, duns: str, error_text: str) -> None:
        self._mark_failed("snov_queue", "snov_status", duns=duns, error_text=error_text)
        self.refresh_final_company(duns)
