...
from .store import EnglandKompassStore


LOGGER = logging.getLogger("england.kompass.pipeline")
CHECKPOINT_NAME = "list_checkpoint.json"
REGION_LINK_RE = re.compile(r'href=(["\'])(?P<href>/z/gb/r/[^"\']+)\1', re.I)
COUNTY_LINK_RE = re.compile(r'href=(["\'])(?P<href>/z/gb/d/[^"\']+)\1', re.I)
...
def run_pipeline_list(
    *,
    ...
    """抓取 Kompass England 多地区入口，仅保留公司名与官网。"""
...
def extract_seed_urls(page_html: str, *, country_code: str, kind: str = "region") -> list[str]:
    """从页面里抽取地区种子 URL。"""
    pattern = COUNTY_LINK_RE if kind == "county" else REGION_LINK_RE
    results: list[str] = []
    seen: set[str] = set()
    for matched in pattern.finditer(str(page_html or "")):
...
def _export_websites(output_dir: Path, store: EnglandKompassStore) -> None:
...
def _build_initial_checkpoint(client: KompassClient, output_dir: Path) -> dict[str, object]:
    root_url = "https://us.kompass.com/businessplace/z/gb/"
    root_html = client.fetch_page(root_url, referer=root_url)
    region_urls = extract_seed_urls(root_html, country_code="gb")
    seeds = [{"url": url, "label": _seed_label(url), "page": 0, "status": "pending"} for url in region_urls]
    for region_url in list(region_urls):
        if "/z/gb/r/england" not in region_url:
            continue
        region_html = client.fetch_page(region_url, referer=root_url)
        for county_url in extract_seed_urls(region_html, country_code="gb", kind="county"):
            seeds.append({"url": county_url, "label": _seed_label(county_url), "page": 0, "status": "pending"})
...
def _seed_label(seed_url: str) -> str:
    ...
    return parts[-2] if len(parts) >= 2 and parts[-1].startswith("gb_") else parts[-1] if parts else "seed"
