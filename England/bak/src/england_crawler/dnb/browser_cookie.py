...
def _build_cookie_header(cookies: list[dict[str, Any]], host: str = DEFAULT_DNB_HOST) -> str:
    now = time.time()
...
def _fetch_cdp_cookies(ws_url: str, timeout_seconds: float) -> list[dict[str, Any]]:
    ...
    try:
        message = {"id": 1, "method": "Storage.getCookies"}
...
def fetch_live_dnb_cookie_header(
    *,
    debug_url: str = DEFAULT_CHROME_DEBUG_URL,
    host: str = DEFAULT_DNB_HOST,
    ...
    return _build_cookie_header(cookies, host=host)
...
def _write_cookie_header_to_env(env_path: Path, cookie_header: str) -> None:
    ...
    new_lines: list[str] = []
    for line in lines:
        if line.startswith("DNB_COOKIE_HEADER="):
            new_lines.append(f"DNB_COOKIE_HEADER={cookie_header}")
            replaced = True
            continue
...
def resolve_dnb_cookie_header(
    *,
    ...
    debug_url = os.getenv("DNB_CHROME_DEBUG_URL", DEFAULT_CHROME_DEBUG_URL).strip() or DEFAULT_CHROME_DEBUG_URL
    try:
    ...
    if live_cookie != fallback:
        ...
        os.environ["DNB_COOKIE_HEADER"] = live_cookie
        log.info("已从 9222 浏览器刷新 DNB_COOKIE_HEADER。")
    return live_cookie


class DnbCookieProvider:
    """运行时按需刷新 DNB cookie。"""
