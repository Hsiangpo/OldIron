...
class DetailQueueStore:
    ...
    def _init_schema(self) -> None:
        with self._lock:
            ...
            self._conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_detail_queue_claim ON detail_queue(status, next_run_at, updated_at, duns)"
            )
            self._conn.commit()
    ...
    def mark_failed(self, duns: str, *, retries: int, error_text: str) -> None:
        with self._lock:
            def _op() -> None:
                self._conn.execute(
                    "UPDATE detail_queue SET status = 'failed', retries = ?, last_error = ?, updated_at = ? WHERE duns = ?",
                    (max(retries, 0), error_text[:500], _utc_now(), duns),
                )
                self._conn.commit()
            run_with_sqlite_retry(self._conn, _op)
