...
- Country-specific source overrides:
  ...
  - `UnitedArabEmirates`: `wiza` is the original non-Snov site flow. It uses normal UAE post-list pipelines.
  - `UnitedArabEmirates`: `wizasnov` does not run GMap.
  - `UnitedArabEmirates`: `wizasnov` `P1` only collects `company_name + website + phone` from Wiza protocol list pages; it does not enter Wiza contacts pages.
  - `UnitedArabEmirates`: `wizasnov` `P3` uses shared `Snov` only, not website protocol crawling. It fetches domain emails, prospects, then uses LLM to pick top 4 leaders plus finance and accounting contacts.
  - `UnitedArabEmirates`: `wizasnov` delivery output columns are `company_name, website, people_json, emails, phone`.
  - `UnitedArabEmirates`: `wizasnov` delivery gate requires `company_name + people_json + emails`; `phone` may be empty.
