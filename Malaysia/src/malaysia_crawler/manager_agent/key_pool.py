...
class FirecrawlKeyPool:
    ...
    def __init__(
        ...
        *,
        ...
        self._keys = [item.strip() for item in keys if item.strip()]
        if not self._keys:
            raise ValueError("firecrawl keys 为空。")
    ...
    def _init_db(self) -> None:
        with self._lock:
            ...
            try:
                ...
                # 中文注释：进程异常退出时可能遗留 in_flight 计数，启动时统一清零，避免键位被永久占满。
        ...
        with self._lock:
            ...
            try:
                ...
                conn.execute(
                    """
                    UPDATE keys
                    SET in_flight = in_flight + 1, last_used = ?
                    WHERE key = ?
                    """,
                    (now, key),
                )
        ...
        with self._lock:
            ...
            try:
                conn.execute(
                    """
                    UPDATE keys
                    SET failure_count = 0, state = 'active', cooldown_until = NULL, disabled_reason = NULL
                    WHERE key = ?
                    """,
                    (lease.key,),
                )
        ...
        with self._lock:
            ...
            try:
                conn.execute(
                    """
                    UPDATE keys
                    SET failure_count = failure_count + 1, state = 'cooldown', cooldown_until = ?, disabled_reason = NULL
                    WHERE key = ?
                    """,
                    (until, lease.key),
                )
            ...
            try:
                ...
                if next_count >= max(self._config.failure_threshold, 1):
                    ...
                    conn.execute(
                        """
                        UPDATE keys
                        SET failure_count = ?, state = 'cooldown', cooldown_until = ?, disabled_reason = NULL
                        WHERE key = ?
                        """,
                    ...
                    conn.execute(
                        """
                        UPDATE keys
                        SET failure_count = ?, disabled_reason = NULL
                        WHERE key = ?
                        """,
        ...
        with self._lock:
            ...
            try:
                conn.execute(
                    """
                    UPDATE keys
                    SET state = 'disabled', disabled_reason = ?, cooldown_until = NULL, in_flight = 0
                    WHERE key = ?
                    """,
                    (reason, lease.key),
                )
