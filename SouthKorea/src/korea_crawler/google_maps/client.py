...
MAP_HEADERS = {
    ...
    "referer": "https://www.google.com/maps?hl=ko&gl=kr",
}
...
class GoogleMapsClient:
    ...
    def search_company_profile(
        ...
        if picked is None:
            return GoogleMapsPlaceResult()
        return GoogleMapsPlaceResult(
            company_name=str(picked.get("company_name_local", "")),
            phone=picked["phone"],
...
def _normalize_local_candidate(value: str) -> str:
    text = _normalize_text(value)
    text = re.sub(r"\((소유자|사업주)\)", "", text).strip()
...
def _extract_candidate_local_name(node: Any) -> str:
    ...
    for item in _flatten_strings(node):
        normalized = _normalize_local_candidate(item)
        if not normalized:
            continue
        score = _local_name_score(normalized)
        if score <= 0:
            continue
        current_score, current_count = candidates.get(normalized, (score, 0))
        candidates[normalized] = (max(current_score, score), current_count + 1)
    if not candidates:
        return ""
    ranked = sorted(
        candidates.items(),
        key=lambda entry: (
            entry[1][0] + min(entry[1][1] - 1, 3) * 20,
            entry[1][1],
            len(entry[0]),
        ),
        reverse=True,
...
def _pick_best_candidate(
    ...
    if int(scored[0]["score"]) < MIN_CANDIDATE_SCORE:
        return None
    local_name = str(scored[0].get("company_name_local", ""))
    if local_name and _local_name_score(local_name) <= 0:
        return None
    return scored[0]
