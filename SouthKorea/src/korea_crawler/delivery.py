...
def _build_key(company_name: str, domain: str) -> str:
    ...
    name_norm = re.sub(r"[^a-z0-9가-힣]+", "", company_name.lower())
...
def _list_existing_days(delivery_root: Path) -> list[int]:
    ...
    for item in delivery_root.iterdir():
        if not item.is_dir():
            continue
        matched = re.fullmatch(r"SouthKorea_day(\d{3})", item.name)
        if matched:
...
def _read_baseline_keys(delivery_root: Path, baseline_day: int) -> set[str]:
    ...
    if baseline_day <= 0:
        return set()
    baseline_dir = delivery_root / f"SouthKorea_day{baseline_day:03d}"
...
def build_delivery_bundle(
    ...
    # 构建交付目录
    day_dir = delivery_root / f"SouthKorea_day{target_day:03d}"
    ...
    with csv_path.open("w", encoding="utf-8", newline="") as fp:
        writer = csv.DictWriter(
            fp,
            fieldnames=["company_name", "ceo", "homepage", "emails"],
        )
        writer.writeheader()
