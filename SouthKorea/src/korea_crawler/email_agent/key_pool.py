"""Firecrawl Key 池（同步版，复用 Malaysia 方案）。"""
...
class FirecrawlKeyPool:
    ...
    def mark_rate_limited(self, lease: KeyLease, retry_after: float | None = None) -> None:
        ...
        with self._lock:
            ...
            try:
                conn.execute(
                    "UPDATE keys SET failure_count = failure_count + 1, state = 'cooldown', cooldown_until = ? WHERE key = ?",
                    (until, lease.key),
                )
                conn.commit()
            finally:
        ...
        with self._lock:
            ...
            try:
                ...
                if next_count >= max(self._config.failure_threshold, 1):
                    ...
                    conn.execute(
                        "UPDATE keys SET failure_count = ?, state = 'cooldown', cooldown_until = ? WHERE key = ?",
    ...
    def _remove_key_from_file(self, target_key: str) -> None:
        ...
        for raw in self._key_file.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            kept.append(line)
        text = "\n".join(kept).strip()
        self._key_file.write_text((text + "\n") if text else "", encoding="utf-8")
