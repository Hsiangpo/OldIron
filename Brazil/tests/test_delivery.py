...
class DeliveryTests(unittest.TestCase):
    def test_day1_outputs_only_qualified_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue((root / "delivery" / "Brazil_day001" / "companies.csv").exists())
            self.assertTrue((root / "delivery" / "Brazil_day001" / "keys.txt").exists())
    ...
    def test_day1_keeps_same_name_with_different_representative_as_two_records(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse((root / "delivery" / "Brazil_day001" / "dnb.csv").exists())
