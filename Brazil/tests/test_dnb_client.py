...
class DnbClientTests(unittest.TestCase):
    def test_refresh_cookies_debounces_forced_browser_snapshot(self) -> None:
        ...
        with mock.patch("brazil_crawler.sites.dnb.client.time.monotonic", return_value=100.0):
            self.assertTrue(client.refresh_cookies(force=True))
        with mock.patch("brazil_crawler.sites.dnb.client.time.monotonic", return_value=110.0):
            self.assertFalse(client.refresh_cookies(force=True))
        with mock.patch("brazil_crawler.sites.dnb.client.time.monotonic", return_value=170.0):
            self.assertTrue(client.refresh_cookies(force=True))
    ...
    def test_browser_cookie_provider_reuses_disk_snapshot(self) -> None:
        from brazil_crawler.sites.dnb.client import DnbBrowserCookieProvider
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                mock.patch.object(
                    ...
                    return_value=(
                        [{"name": "akaas_br", "value": "cookie1", "domain": ".dnb.com", "path": "/"}],
                        DnbBrowserHeaders(
                            user_agent="ua",
                            sec_ch_ua='"Chromium";v="146"',
                            sec_ch_ua_platform='"macOS"',
