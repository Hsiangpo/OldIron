...
class DnbBrStore:
    ...
    def _init_tables(self) -> None:
        ...
        conn.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_br_dnb_segments_status_segment
            ON dnb_segments(status, segment_id);
            CREATE INDEX IF NOT EXISTS idx_br_detail_queue_status_updated
            ON detail_queue(status, updated_at, duns);
            CREATE INDEX IF NOT EXISTS idx_br_gmap_queue_status_updated
            ON gmap_queue(status, updated_at, duns);
            CREATE INDEX IF NOT EXISTS idx_br_site_queue_status_updated
            ON site_queue(status, updated_at, duns);
            CREATE INDEX IF NOT EXISTS idx_br_companies_detail_status
            ON companies(detail_status, updated_at, duns);
            CREATE INDEX IF NOT EXISTS idx_br_companies_gmap_status
            ON companies(gmap_status, updated_at, duns);
            CREATE INDEX IF NOT EXISTS idx_br_companies_site_status
            ON companies(site_status, updated_at, duns);
            CREATE INDEX IF NOT EXISTS idx_br_companies_name_norm
            ON companies(lower(trim(company_name)));
            CREATE INDEX IF NOT EXISTS idx_br_final_name_norm
    ...
    def purge_bad_websites(self) -> int:
        def _action(conn: sqlite3.Connection) -> int:
            ...
            conn.execute(
                f"DELETE FROM site_queue WHERE duns IN ({placeholders})",
                bad_duns,
            )
            conn.execute(
                f"DELETE FROM final_companies WHERE duns IN ({placeholders})",
                bad_duns,
            )
            return len(bad_duns)
