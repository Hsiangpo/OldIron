...
def _gmap_worker(store: DnbUsStore, stop_event) -> None:
    client = GoogleMapsClient(GoogleMapsConfig(hl="en", gl="us"))
    while not stop_event.is_set():
        ...
        try:
            result = client.search_company_profile(
                f"{task.company_name} {task.address} {task.city} {task.region} United States",
                task.company_name,
            )
