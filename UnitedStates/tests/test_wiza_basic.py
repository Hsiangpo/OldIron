...
from unitedstates_crawler.delivery import build_delivery_bundle
from unitedstates_crawler.sites.wiza.client import COMPANY_FILTER
from unitedstates_crawler.sites.wiza.pipeline import run_pipeline_list as run_wiza_pipeline_list
from unitedstates_crawler.sites.wiza.store import UnitedStatesWizaStore


class UnitedStatesWizaTests(unittest.TestCase):
    def test_company_filter_targets_united_states(self) -> None:
        self.assertEqual(COMPANY_FILTER["v"], "united states")

    def test_wiza_list_usage_limit_still_exports_current_websites_txt(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            output_dir = Path(tmp_dir)
            store = UnitedStatesWizaStore(output_dir / "companies.db")
                ...
                def search_companies(self, *, search_after, page_size):
                    ...
                    if self._calls == 1:
                        return SimpleNamespace(
                            items=[{"name": "Example Inc", "website": "example.com"}],
                            total=0,
                            total_relation="gte",
                            page_size=100,
                            last_sort=["next-page"],
            ...
            with patch("unitedstates_crawler.sites.wiza.pipeline.WizaClient", FakeClient):
        ...
        self.assertEqual(lines, ["https://example.com"])
    ...
    def test_websites_delivery_uses_independent_day_sequence(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            (delivery_root / "UnitedStates_day001").mkdir(parents=True, exist_ok=True)

            summary = build_delivery_bundle(data_root, delivery_root, "day1", delivery_kind="websites")

            package_dir = delivery_root / "UnitedStates_websites_day001"
