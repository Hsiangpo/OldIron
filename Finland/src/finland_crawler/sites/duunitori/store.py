...
class DuunitoriDetailTask:
    job_id: str
    url: str
    retries: int = 0

@dataclass(slots=True)
class DuunitoriGMapTask:
    job_id: str
    company_name: str
    city: str
    retries: int = 0
...
class DuunitoriStore:
    ...
    def upsert_job(self, job: dict[str, str], page: int = 0) -> None:
        ...
        with self._lock:
            self._conn.execute(
                """INSERT INTO jobs
                (job_id, url, title, company_name, city, source_page, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ...
            self._conn.execute(
                ...
                (job_id, url, status, retries, next_run_at, last_error, updated_at)
                VALUES (?, ?, 'pending', 0, ?, '', ?)""",
                (job_id, url, now, now),
            )
            self._conn.commit()
    ...
    def upsert_job_detail(self, job_id: str, detail: dict[str, str]) -> None:
        ...
        with self._lock:
            ...
            row = self._conn.execute(
                "SELECT company_name FROM jobs WHERE job_id = ?", (job_id,)
            ).fetchone()
            ...
            self._conn.execute(
                ...
                (email, phone, representative,
                 detail.get("salary", ""), detail.get("description", "")[:2000],
                 now, job_id),
            )
            if deliverable:
                self._insert_final(job_id, company_name, representative, email, phone, "", "", "duunitori-direct")
            else:
                self._conn.execute(
    ...
    def claim_detail_task(self) -> DuunitoriDetailTask | None:
        now = _utc_now()
        with self._lock:
            row = self._conn.execute(
            ...
            self._conn.commit()
            return DuunitoriDetailTask(job_id=row["job_id"], url=row["url"], retries=row["retries"])

    def mark_detail_done(self, job_id: str) -> None:
        now = _utc_now()
        with self._lock:
            self._conn.execute(
                "UPDATE detail_queue SET status = 'done', updated_at = ? WHERE job_id = ?", (now, job_id),
            )
            self._conn.commit()
    ...
    def claim_gmap_task(self) -> DuunitoriGMapTask | None:
        now = _utc_now()
        with self._lock:
            row = self._conn.execute(
    ...
    def claim_firecrawl_task(self) -> DuunitoriFirecrawlTask | None:
        now = _utc_now()
        with self._lock:
            row = self._conn.execute(
            ...
            if has_email and next_rep:
                for em in merged:
                    self._insert_final(
                        ...
                        str(evidence_url or ""), "duunitori-firecrawl",
                    )
            self._conn.commit()
    ...
    def get_progress(self) -> DuunitoriProgress:
        with self._lock:
    ...
    def export_jsonl_snapshots(self, output_dir: Path) -> None:
        ...
        with self._lock:
            ...
            _write_jsonl_atomic(output_dir / "duunitori_jobs.jsonl", [dict(r) for r in rows])
            rows = self._conn.execute("SELECT * FROM final_companies ORDER BY job_id").fetchall()
            _write_jsonl_atomic(output_dir / "duunitori_final.jsonl", [dict(r) for r in rows])
