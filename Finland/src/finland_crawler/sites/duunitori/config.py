...
class DuunitoriConfig:
    ...
    max_list_pages: int = 500     # Duunitori 最多翻 500 页
    request_delay: float = 1.0    # 列表页翻页间隔
    ...
    def from_env(
        ...
        *,
        ...
        store_db = output_dir / "duunitori_store.db"
        proxy = _env_str("DUUNITORI_PROXY_URL", _env_str("PROXY_URL", "http://127.0.0.1:7897"))
        return cls(
            project_root=project_root,
            output_dir=output_dir,
            store_db_path=store_db,
            proxy_url=proxy,
            timeout_seconds=_env_float("DUUNITORI_TIMEOUT", 30.0),
            detail_workers=_env_int("DUUNITORI_DETAIL_WORKERS", detail_workers),
            gmap_workers=_env_int("DUUNITORI_GMAP_WORKERS", gmap_workers),
            firecrawl_workers=_env_int("DUUNITORI_FIRECRAWL_WORKERS", firecrawl_workers),
            ...
            firecrawl_task_max_retries=_env_int("DUUNITORI_FIRECRAWL_TASK_MAX_RETRIES", 5),
        )
