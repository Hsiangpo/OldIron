...
from finland_crawler.sites.jobly.pipeline import run_jobly_pipeline
    ...
    for _ in range(8):
        try:
            ...
            if lock_pid and _pid_exists(lock_pid):
                raise RuntimeError(f"已有运行中的 Jobly 进程: PID={lock_pid}")
            try:
                lock_path.unlink()
            except OSError:
                time.sleep(0.2)
        ...
        return lock_path
    raise RuntimeError("无法获取 Jobly 运行锁")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="芬兰 Jobly 爬虫")
...
def run_jobly(argv: list[str]) -> int:
    """Jobly CLI 主入口。"""
    ...
    output_dir = Path(args.output_dir).resolve() if str(args.output_dir).strip() else ROOT / "output" / "jobly"
    ...
    client = JoblyClient(timeout_seconds=config.timeout_seconds, proxy_url=config.proxy_url)

    try:
        run_jobly_pipeline(config, client, skip_gmap=bool(args.skip_gmap), skip_firecrawl=bool(args.skip_firecrawl))
        return 0
    finally:
        if gmap_auto_started:
            _stop_gmap_backend()
