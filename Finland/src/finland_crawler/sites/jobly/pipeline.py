"""Jobly 主流程 — 列表页翻页 → 详情 → GMap → Protocol+LLM。"""
...
from finland_crawler.sites.jobly.client import JoblyClient
from finland_crawler.sites.jobly.config import JoblyConfig
from finland_crawler.sites.jobly.store import (
    JoblyDetailTask, JoblyGMapTask, JoblyFirecrawlTask, JoblyStore,
)
...
def _gmap_queries(task: JoblyGMapTask) -> list[str]:
    queries: list[str] = []
    for value in (
...
class JoblyPipelineRunner:
    """Jobly 四阶段流水线。"""

    def __init__(
        self, *, config: JoblyConfig, client: JoblyClient,
        ...
        self.store = JoblyStore(config.store_db_path)
        ...
        self._firecrawl_domain_cache = FirecrawlDomainCache(
            config.project_root / "output" / "jobly_firecrawl_cache.db"
        )
    ...
    def run(self) -> None:
        ...
        if recovered:
            LOGGER.info("Jobly 已回收陈旧任务：%s", recovered)
        workers = self._build_workers()
        for w in workers:
            w.start()
            if w.name.startswith("jbl-email-"):
                time.sleep(0.1)
        ...
        def _bg_search() -> None:
            try:
                ...
                LOGGER.error("Jobly 搜索异常：%s", exc)
            finally:
                self.search_done.set()

        threading.Thread(target=_bg_search, name="jbl-search", daemon=True).start()
        try:
            self._monitor()
        finally:
            self.stop_event.set()
    ...
    def _run_search(self) -> None:
        ...
        page = pages_done      # Jobly 从 0 开始
        LOGGER.info("Jobly 搜索开始：从第 %d 页续跑", page)
        total_collected = 0
        while not self.stop_event.is_set():
            try:
                jobs, has_next = self.client.fetch_list_page(page)
            except Exception as exc:
                LOGGER.warning("Jobly 列表第 %d 页失败：%s", page, exc)
                time.sleep(5)
                continue
            for job in jobs:
                self.store.upsert_job(job, page)
            total_collected += len(jobs)
            self.store.update_search_progress(total_collected, page + 1)
            LOGGER.info("Jobly 搜索：page=%d rows=%d total=%d", page, len(jobs), total_collected)
    ...
    def _build_workers(self) -> list[threading.Thread]:
        workers = [
            threading.Thread(target=self._detail_worker, name=f"jbl-detail-{i+1}", daemon=True)
            for i in range(self.config.detail_workers)
        ]
        if not self.skip_gmap:
            workers.extend(
                threading.Thread(target=self._gmap_worker, name=f"jbl-gmap-{i+1}", daemon=True)
                for i in range(self.config.gmap_workers)
            )
        if not self.skip_firecrawl:
            workers.extend(
                threading.Thread(target=self._email_worker, name=f"jbl-email-{i+1}", daemon=True)
    ...
    def _monitor(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            if now - last_log >= 10.0:
                ...
                LOGGER.info(
                    "J 进度：search=%d detail=%d/%d gmap=%d+%d email=%d+%d jobs=%d final=%d",
    ...
    def _do_gmap(self, task: JoblyGMapTask) -> None:
        try:
    ...
    def _process_firecrawl_task(self, task: JoblyFirecrawlTask) -> None:
        if not task.website:
    ...
    def _handle_firecrawl_failure(self, task: JoblyFirecrawlTask, exc: Exception) -> None:
...
def run_jobly_pipeline(
    config: JoblyConfig, client: JoblyClient,
    *, skip_gmap: bool = False, skip_firecrawl: bool = False,
) -> None:
    runner = JoblyPipelineRunner(
