...
from finland_crawler.sites.tyomarkkinatori.pipeline import run_tmt_pipeline
    ...
    for _ in range(8):
        try:
            ...
            if lock_pid and _pid_exists(lock_pid):
                raise RuntimeError(f"已有运行中的 TMT 进程: PID={lock_pid}")
            try:
                lock_path.unlink()
            except OSError:
                time.sleep(0.2)
        ...
        return lock_path
    raise RuntimeError("无法获取 TMT 运行锁")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="芬兰 Työmarkkinatori 爬虫")
    parser.add_argument("--output-dir", default="", help="输出目录")
    parser.add_argument("--detail-workers", type=int, default=8, help="详情并发数")
    parser.add_argument("--gmap-workers", type=int, default=64, help="Google Maps 并发数")
    parser.add_argument("--email-workers", dest="firecrawl_workers", type=int, default=8, help="邮箱补充并发数")
...
def run_tmt(argv: list[str]) -> int:
    """Työmarkkinatori CLI 主入口。"""
    ...
    output_dir = Path(args.output_dir).resolve() if str(args.output_dir).strip() else ROOT / "output" / "tyomarkkinatori"
