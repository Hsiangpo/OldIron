    ...
    async def acquire(self) -> KeyLease:
        while True:
            ...
            if lease is not None:
                return lease
            if not any(entry.state != KeyState.DISABLED for entry in self._entries):
...
class _SharedKeyPool:
    ...
    async def acquire(self) -> KeyLease:
        while True:
            ...
            if lease is not None:
                return lease
            if not await asyncio.to_thread(self._has_enabled_key_sync):
                raise RuntimeError("no available firecrawl key")
            await asyncio.sleep(self._config.check_interval)

    def _has_enabled_key_sync(self) -> bool:
        conn = self._connect()
        try:
            if not self._keys:
                return False
            ...
            row = conn.execute(
                f"SELECT 1 FROM keys WHERE state != ? AND key IN ({placeholders}) LIMIT 1",
                (KeyState.DISABLED.value, *self._keys),
            ).fetchone()
