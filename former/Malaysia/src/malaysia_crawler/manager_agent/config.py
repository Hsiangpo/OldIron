...
class ManagerAgentConfig:
    ...
    @classmethod
    def from_env(cls, project_root: Path) -> "ManagerAgentConfig":
        firecrawl_keys_file = Path(
        ...
        return cls(
            ...
            firecrawl_key_cooldown_seconds=_env_int(
                "FIRECRAWL_KEY_COOLDOWN_SECONDS", 90
            ),
            firecrawl_key_failure_threshold=_env_int(
                "FIRECRAWL_KEY_FAILURE_THRESHOLD", 5
            ),
            map_include_subdomains=(
                os.getenv("MANAGER_AGENT_MAP_INCLUDE_SUBDOMAINS", "true")
                .strip()
                .lower()
                in {"1", "true", "yes", "y"}
            ),
            map_limit=_env_int("MANAGER_AGENT_MAP_LIMIT", 8000),
            max_rounds=_env_int("MANAGER_AGENT_MAX_ROUNDS", 3),
            pick_per_round=_env_int("MANAGER_AGENT_PICK_PER_ROUND", 5),
            fetch_per_round=_env_int("MANAGER_AGENT_FETCH_PER_ROUND", 5),
            retry_backoff_seconds=_env_float(
                "MANAGER_AGENT_RETRY_BACKOFF_SECONDS", 30.0
            ),
